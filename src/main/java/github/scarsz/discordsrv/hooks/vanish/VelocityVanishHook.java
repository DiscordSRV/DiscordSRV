...
public class VelocityVanishHook implements VanishHook {


    @EventHandler(priority = EventPriority.MONITOR)
    public void onPlayerShow(PostVanishEvent event) {
        if (!Settings.INSTANCE.getFakeJoinLeaveMessage()) {
            return;
        }

        final Player player = event.getPlayer();

        String joinMessage = Settings.INSTANCE.formatMessage(Message.DISCORDSRV_QUIT_MESSAGE, new TextReplacement("player", player.getName()));
    ...
    @EventHandler(priority = EventPriority.MONITOR)
    public void onPlayerHide(PostUnVanishEvent event) {
        if (!Settings.INSTANCE.getFakeJoinLeaveMessage()) {
            return;
        }
        ...
        String joinMessage = Settings.INSTANCE.formatMessage(Message.DISCORDSRV_JOIN_MESSAGE, new TextReplacement("player", player.getName()));
    ...
    public Plugin getPlugin() {
        return PluginUtil.getPlugin("VelocityVanish");
    }
