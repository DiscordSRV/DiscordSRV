...
public class DiscordSRV extends JavaPlugin {
        ...
        MessageAction.setDefaultMentions(config().getStringList("DiscordChatChannelAllowedMentions").stream()
                .map(s -> {
                    try {
                        ...
                        DiscordSRV.error(
                                "Unknown mention type \"" + s + "\" defined in DiscordChatChannelAllowedMentions");
                        return null;
                    }
    ...
    public void init() {
        ...
        if (!isUpdateCheckDisabled()) {
            ...
            updateChecker.scheduleAtFixedRate(() -> DiscordSRV.updateIsAvailable = UpdateUtil.checkForUpdates(false),
                    6, 6, TimeUnit.HOURS);
        }
        ...
        // Limit okhttp to 20 concurrent requests to avoid hogging every available
        // thread
        ...
        if (proxyHost != null && !proxyHost.isEmpty() && authPassword == null && authPassword.isEmpty()) {
            ...
            httpClient = new OkHttpClient.Builder()
                    ...
                    .build();
        } else if (proxyHost != null && !proxyHost.isEmpty() && authPassword != null && !authPassword.isEmpty()) {
            System.setProperty("jdk.http.auth.tunneling.disabledSchemes", "");
            // Proxy with password
            httpClient = new OkHttpClient.Builder()
                    .proxy(new Proxy(Proxy.Type.HTTP, new InetSocketAddress(proxyHost, Integer.parseInt(proxyPort))))
            ...
            if (throwable instanceof HierarchyException) {
                ...
                if (cause instanceof InterruptedIOException && jda != null) {
                    ...
                    if (status == JDA.Status.SHUTDOWN || status == JDA.Status.SHUTTING_DOWN) {
                        // Ignore InterruptedIOException's during shutdown, we can't hold up the server
                        // from stopping forever,
                        // so some requests are cancelled during shutdown. Logging errors for those
                        // request failures isn't important.
                        return;
                    }
                }
        ...
        // schedule slash commands to be updated later when plugins are ready (once the
        // server had started up completely)
            ...
            try {
                ...
                do {
                    stringBuilder.append("\n").append("Caused by: ")
                            .append(selected instanceof UnknownHostException ? "UnknownHostException"
        ...
        // Bukkit.getMessenger().registerIncomingPluginChannel(this, "lunarclient:pm",
        // incompatibleClientManager);
        ...
        if (PluginUtil.pluginHookIsEnabled("PlaceholderAPI")) {
            try {
                DiscordSRV.info(
                        LangUtil.InternalMessage.PLUGIN_HOOK_ENABLING.toString().replace("{plugin}", "PlaceholderAPI"));
                SchedulerUtil.runTask(this, () -> {
                    try {
                        if (me.clip.placeholderapi.PlaceholderAPIPlugin.getInstance().getLocalExpansionManager()
                                .findExpansionByIdentifier("discordsrv").isPresent()) {
                ...
                if (!SchedulerUtil.isFolia()) { // not implemented yet on folia
                    for (BukkitWorker activeWorker : Bukkit.getScheduler().getActiveWorkers()) {
                        if (activeWorker.getOwner().equals(this)) {
                            ...
                            warning("a DiscordSRV scheduler task still active during onDisable: "
                                    + stackTrace.remove(0));
                            debug(stackTrace);
                        }
                    }
    ...
    public void processChatMessage(Player player, Component message, String channel, boolean cancelled,
        ...
        if (!webhookMessageDelivery) {
            ...
            /*
             * // Reserialize the message pattern, in the case the placeholders added more
             * color codes.
             * // DiscordSRV is not ready for this yet. Leaving this here for when that
             * faithful day comes upon us.
             * if (reserializer){
            ...
            // Replace the message after to avoid replacing rouge PAPI placeholders inside
    ...
    /**
     * @return Whether DiscordSRV should disable it's update checker. Doing so is
    ...
    /**
     * @return Whether DiscordSRV group role synchronization has been enabled in the
     *         configuration.
     */
